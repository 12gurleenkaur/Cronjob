...
delete_job() {
    ...
    crontab -l > "$TMP" 2>/dev/null || { dialog --msgbox "No jobs to delete." 6 30; return; }
    
    # backup before doing anything risky
    backup_jobs
    
    # number each line so user can pick
    nl -ba "$TMP" > "$DIR/numbered.txt"
    dialog --textbox "$DIR/numbered.txt" 20 70

    # ask for which line number to delete
    dialog --inputbox "Line to delete:" 8 40 2>"$DIR/del.tmp" || return
    L=$(cat "$DIR/del.tmp")
    rm "$DIR/del.tmp"
    ...
    sed -i "${L}d" "$TMP"
    crontab "$TMP"
    echo "$(date): Deleted line $L" >> "$LOG"
    
    dialog --msgbox "Job deleted!" 6 30
}
